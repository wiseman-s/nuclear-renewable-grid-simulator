...
def generate_pdf_report(scenario_name, dispatch_df=None, extra_metrics=None):
    ...
    story.append(
        Paragraph(
            f"NRGS Scenario Report — {scenario_name}",
            styles["Title"]
        )
    )
    ...
    story.append(
        Paragraph(
            "Nuclear & Renewable Grid Simulator • Kenya",
            styles["Normal"]
        )
    )
    ...
    if dispatch_df is not None:
        story.append(
            Paragraph(
                "Dispatch Summary (First 12 hours + Totals)",
                styles["Heading2"]
            )
        )

        numeric_df = dispatch_df.select_dtypes(
            include="number"
        ).head(12)
...
if page == "🏠 Home":
    ...
    st.markdown(
        "**v2.1 MVP** — Supporting Kenya's clean energy transition"
    )
    ...
    st.info(
        "Purely computational → safe, educational, policy-oriented."
    )
...
# ─────────────────────────────────────────────────────────────
    ...
    st.header(
        "⚛️ Reactor Transient Simulation"
    )

    st.caption(
        "6-group point kinetics + thermal feedback"
    )
    ...
    with col1:
        ...
        if scenario == "Step Reactivity Insertion":
            ...
            result = step_reactivity_insertion(
                rho_step=rho,
                t_insert=t_ins,
...
# ─────────────────────────────────────────────────────────────
    ...
    st.header(
        "24-Hour Merit-Order Dispatch"
    )

    include_nuc = st.checkbox(
        "Include 1 GW Nuclear",
        value=True
    )

    demand_df = pd.read_csv(
        "data/kenya_load_profile.csv"
    )
    ...
    numeric_cols = dispatch_df.select_dtypes(
        include="number"
    ).columns
        ...
        if dispatch_df[col].sum() > 0:
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    y=dispatch_df[col],
                    name=col.capitalize(),
                    marker_color=source_colors.get(
                        col,
                        "#AAAAAA"
                    )
                )
    ...
    if (
        "unserved_mw" in dispatch_df.columns
        and dispatch_df["unserved_mw"].sum() > 0
    ):

        fig.add_trace(
            go.Bar(
                x=dispatch_df.index,
                y=dispatch_df["unserved_mw"],
                name="Unserved",
                marker_color=source_colors["unserved_mw"],
                opacity=0.85
            )
        )
    ...
    unserved_mwh = dispatch_df.get(
        "unserved_mw",
        pd.Series(0)
    ).sum()

    unserved_pct = (
        (unserved_mwh / (total_mwh + unserved_mwh)) * 100
        if total_mwh > 0
        else 0
    )
    ...
    numeric_disp = dispatch_df.select_dtypes(
        "number"
    )

    # Streamlit 1.62+ compatible
    st.dataframe(
        numeric_disp.style.format("{:,.0f}"),
        width="stretch"
    )

    if st.button(
        "Export Grid Report (PDF)"
    ):

        metrics = {
            "Nuclear Included": (
                "Yes" if include_nuc else "No"
            ),
...
# ─────────────────────────────────────────────────────────────
    ...
    if st.button(
        ...
        fname = (
            f"NRGS_"
            f"{scenario.replace(' ', '_').replace('(', '').replace(')', '')}_"
...
# ─────────────────────────────────────────────────────────────
    ...
    nuc_year = st.slider(
        "Nuclear online after (years)",
        5,
        12,
        8
    )

    plan_df = long_term_plan(
        years=years,
        annual_growth=growth,
        nuclear_commission_year=nuc_year
    )
    ...
    fig.update_layout(
        title="Demand vs Nuclear Capacity",
        xaxis_title="Year",
        yaxis_title="MW",
        height=550
    )
